...
export class DocumentsController {
  ...
  async batchOCRAlias(
    @Body()
    body: { documentIds: string[]; language?: string; provider?: OCRProvider },
    @Request() req: any,
  ) {
      ...
      if (value.startsWith("[")) {
        try {
          ...
          throw new BadRequestException(
            "Invalid tags format. Expected JSON array or comma-separated string.",
          );
        }
      }
  ...
  async searchDocuments(@Query("query") query: string, @Request() req) {
  ...
  async getRecentDocuments(@Query("limit") limit?: string, @Request() req?) {
  ...
  async getDocumentsByModule(
    ...
    return this.documentsService.getDocumentsByModule(
      module,
      referenceId,
      req.user.userId,
      req.user.role,
  ...
  async getStorageUsage(@Request() req) {
    return this.documentsService.getStorageUsage(
      req.user.userId,
      req.user.role,
    );
  }
  ...
  async batchDelete(@Body() body: { documentIds: string[] }, @Request() req) {
    return this.documentsService.batchDelete(
      body.documentIds,
      req.user.userId,
      req.user.role,
    );
  ...
  async batchAddTags(
    ...
    return this.documentsService.batchAddTags(
      body.documentIds,
      body.tags,
      req.user.userId,
      req.user.role,
  ...
  async getSpecificVersion(
    ...
    return this.documentsService.getSpecificVersion(
      id,
  ...
  async generatePurchaseOrderPDF(
    ...
    const pdfBuffer = await this.pdfGeneratorService.generatePurchaseOrderPDF(
      poId,
      options,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="purchase-order-${poId}.pdf"`,
    );
    res.send(pdfBuffer);
  }
  ...
  async generateExpenseReportPDF(
    ...
    const pdfBuffer = await this.pdfGeneratorService.generateExpenseReportPDF(
      expenseId,
      options,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="expense-report-${expenseId}.pdf"`,
    );
    res.send(pdfBuffer);
  }
  ...
  async generateProjectReportPDF(
    ...
    const pdfBuffer = await this.pdfGeneratorService.generateProjectReportPDF(
      projectId,
      options,
    );
  ...
  async generateSafetyReportPDF(
    ...
    const pdfBuffer = await this.pdfGeneratorService.generateSafetyReportPDF(
      incidentId,
      options,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="safety-report-${incidentId}.pdf"`,
    );
    res.send(pdfBuffer);
  }
  ...
  async generateCustomPDF(
    ...
    const pdfBuffer = await this.pdfGeneratorService.generateCustomPDF(
      body.data,
      body.options || {},
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="custom-document.pdf"`,
    );
    res.send(pdfBuffer);
  }
  ...
  async saveGeneratedPDF(
    ...
    switch (body.documentType) {
      case "invoice":
        pdfBuffer = await this.pdfGeneratorService.generateInvoicePDF(
          body.entityId,
          body.options || {},
        );
        break;
      case "purchase-order":
        pdfBuffer = await this.pdfGeneratorService.generatePurchaseOrderPDF(
          body.entityId,
          body.options || {},
        );
        break;
      case "expense-report":
        pdfBuffer = await this.pdfGeneratorService.generateExpenseReportPDF(
          body.entityId,
          body.options || {},
        );
        break;
      case "project-report":
        pdfBuffer = await this.pdfGeneratorService.generateProjectReportPDF(
          body.entityId,
          body.options || {},
        );
        break;
      case "safety-report":
        pdfBuffer = await this.pdfGeneratorService.generateSafetyReportPDF(
          body.entityId,
          body.options || {},
        );
        break;
      case "custom":
        pdfBuffer = await this.pdfGeneratorService.generateCustomPDF(
          body.data || {},
          body.options || {},
        );
        break;
  ...
  async checkSignatureRequirement(@Param("id") id: string, @Request() req) {
  ...
  @Delete(":id/security")
  @Roles(UserRole.SUPER_ADMIN, UserRole.CEO, UserRole.CFO)
  async removeDocumentSecurity(@Param("id") id: string, @Request() req) {
  ...
  async verifyDocumentPassword(
    ...
    const isValid = await this.securityService.verifyDocumentPassword(
      id,
      body.password,
    );
    return { isValid };
  ...
  @Get(":id/check-access")
  async checkDocumentAccess(@Param("id") id: string, @Request() req) {
