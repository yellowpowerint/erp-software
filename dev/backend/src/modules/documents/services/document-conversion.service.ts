...
type DocumentConversionProvider = (typeof DocumentConversionProvider)[keyof typeof DocumentConversionProvider];
...
export class DocumentConversionService {
  ...
  private async chooseProvider(mimeType: string): Promise<DocumentConversionProvider> {
  ...
  async cancelJob(jobId: string, userId: string) {
    ...
    if (job.status !== DocumentConversionStatus.PENDING && job.status !== DocumentConversionStatus.PROCESSING) {
  ...
  async processJob(jobId: string): Promise<void> {
    ...
    try {
      const { buffer, filename } = await this.convertDocumentToPdfBuffer(document, job.provider);
      ...
      this.logger.log(`Conversion completed for document ${updatedDocument.id} (job ${jobId})`);
    } catch (error: any) {
      const processingTime = Date.now() - startTime;

      const latestJob = await (this.prisma as any).documentConversionJob.findUnique({
        where: { id: jobId },
      });
      ...
      await (this.prisma as any).documentConversionJob.update({
        where: { id: jobId },
        data: {
          status: shouldRetry ? DocumentConversionStatus.PENDING : DocumentConversionStatus.FAILED,
      ...
      if (!shouldRetry) {
        this.logger.error(`Conversion failed for job ${jobId}: ${error?.message || error}`);
      }
  ...
  private async convertDocumentToPdfBuffer(
    ...
    if (provider === DocumentConversionProvider.LOCAL) {
      ...
      try {
        ...
        const buffer = await this.localConvertToPdf(inputBuffer, document.mimeType, document.originalName);
    ...
    if (provider === DocumentConversionProvider.CLOUDCONVERT) {
      return { buffer: await this.cloudConvertToPdf(document), filename: outputFilename };
    }
  ...
  private async localConvertToPdf(
    ...
    throw new BadRequestException('This file type requires an external conversion provider');
  }

  private async convertImageToPdf(inputBuffer: Buffer, mimeType: string): Promise<Buffer> {
  ...
  private async cloudConvertToPdf(document: any): Promise<Buffer> {
    ...
    const isS3 = fileUrl.includes('s3.amazonaws.com') || fileUrl.includes('amazonaws.com');

    if (!isS3) {
      throw new BadRequestException('CloudConvert conversion requires S3 storage or a publicly accessible file URL');
    }
    ...
    const waitResponse = await axios.get(`https://api.cloudconvert.com/v2/jobs/${jobId}/wait`, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
      },
    });
    ...
    if (!outputUrl) {
      throw new BadRequestException('CloudConvert export did not return a download URL');
    }

    const download = await axios.get(outputUrl, { responseType: 'arraybuffer' });
