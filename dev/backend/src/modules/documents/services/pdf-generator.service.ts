...
export class PdfGeneratorService {
   ...
   */
  async generateInvoicePDF(invoiceId: string, options: PDFGenerationOptions = {}): Promise<Buffer> {
    ...
    return this.createPDF(async (doc) => {
      ...
      doc.text(`Date: ${new Date(invoice.createdAt).toLocaleDateString()}`, 50, 165);
      doc.text(`Due Date: ${new Date(invoice.dueDate).toLocaleDateString()}`, 50, 180);
      doc.text(`Status: ${invoice.status}`, 50, 195);

      // Supplier info
      doc.fontSize(12).text('Supplier Information', 50, 230, { underline: true });
      doc.fontSize(10);
      ...
      doc.text(`Created By: ${invoice.createdBy.firstName} ${invoice.createdBy.lastName}`, 50, 460);
      ...
      if (options.includeQRCode) {
        this.addQRCode(doc, options.qrCodeData || `INVOICE-${invoice.invoiceNumber}`, 450, 650);
      }
   ...
   */
  async generatePurchaseOrderPDF(poId: string, options: PDFGenerationOptions = {}): Promise<Buffer> {
    ...
    return this.createPDF(async (doc) => {
      ...
      doc.fontSize(10);
      doc.text(`Name: ${po.createdBy.firstName} ${po.createdBy.lastName}`, 50, 250);
      ...
      doc.text(`Estimated Cost: ₵${po.estimatedCost.toFixed(2)} ${po.currency}`, 50, 365);
      ...
      if (options.includeQRCode) {
        this.addQRCode(doc, options.qrCodeData || `PR-${po.requestNumber}`, 450, 650);
      }
   ...
   */
  async generateProjectReportPDF(projectId: string, options: PDFGenerationOptions = {}): Promise<Buffer> {
    ...
    return this.createPDF(async (doc) => {
      ...
      doc.fontSize(10);
      doc.text('This is a project summary report generated for review.', 50, 220, { width: 500 });
      ...
      if (options.includeQRCode) {
        this.addQRCode(doc, options.qrCodeData || `PROJECT-${projectId}`, 450, 650);
      }
   ...
   */
  async generateExpenseReportPDF(expenseId: string, options: PDFGenerationOptions = {}): Promise<Buffer> {
    ...
    return this.createPDF(async (doc) => {
      ...
      doc.text(`Date: ${new Date(expense.expenseDate).toLocaleDateString()}`, 50, 165);
      doc.text(`Status: ${expense.status}`, 50, 180);

      doc.fontSize(12).text('Submitted By', 50, 215, { underline: true });
      doc.fontSize(10);
      doc.text(`Name: ${expense.submittedBy.firstName} ${expense.submittedBy.lastName}`, 50, 235);
      ...
      doc.text(`Amount: ₵${expense.amount.toFixed(2)} ${expense.currency}`, 50, 360);
      ...
      if (options.includeQRCode) {
        this.addQRCode(doc, options.qrCodeData || `EXPENSE-${expense.expenseNumber}`, 450, 650);
      }
   ...
   */
  async generateSafetyReportPDF(incidentId: string, options: PDFGenerationOptions = {}): Promise<Buffer> {
    ...
    return this.createPDF(async (doc) => {
      ...
      doc.text(`Date: ${new Date(incident.incidentDate).toLocaleDateString()}`, 50, 165);
      ...
      doc.text(`Reported At: ${new Date(incident.reportedAt).toLocaleDateString()}`, 50, 295);
      ...
      if (incident.correctiveActions) {
        doc.fontSize(12).text('Corrective Actions', 50, 500, { underline: true });
        doc.fontSize(9);
      ...
      if (options.includeQRCode) {
        this.addQRCode(doc, options.qrCodeData || `INCIDENT-${incident.incidentNumber}`, 450, 650);
      }
   ...
   */
  async generateCustomPDF(data: any, options: PDFGenerationOptions = {}): Promise<Buffer> {
    return this.createPDF(async (doc) => {
      ...
      if (data.sections) {
        data.sections.forEach((section: any) => {
          doc.fontSize(12).text(section.title, 50, yPosition, { underline: true });
          yPosition += 20;
          doc.fontSize(10);
   ...
   */
  private createPDF(contentCallback: (doc: PDFKit.PDFDocument) => Promise<void> | void): Promise<Buffer> {
  ...
  private async addHeader(doc: PDFKit.PDFDocument, title: string) {
    const companyName = this.configService.get<string>('COMPANY_NAME', 'Mining ERP System');
    ...
    if (logoPath || logoUrl) {
      try {
        ...
        this.logger.warn('Failed to load company logo, using text fallback', error);
    ...
    doc.moveTo(50, yPosition + 30).lineTo(550, yPosition + 30).stroke();
  }
  ...
  private async addQRCode(doc: PDFKit.PDFDocument, data: string, x: number, y: number) {
    try {
      ...
      doc.fontSize(7).text('Scan to verify', x, y + 85, { width: 80, align: 'center' });
    } catch (error) {
