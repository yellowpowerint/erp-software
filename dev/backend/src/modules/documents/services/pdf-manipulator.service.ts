...
export class PdfManipulatorService {
  ...
  async reorderPages(buffer: Buffer, order: number[]): Promise<Buffer> {
    ...
    if (!order || order.length !== total) {
      throw new BadRequestException(
        `order must contain exactly ${total} entries`,
      );
    }
  ...
  async rotatePages(
    ...
    const normalizedRotation =
      this.normalizeRightAngleRotation(rotationDegrees);

    const indices =
      pages && pages.length > 0
        ? this.normalizeUniquePageIndices(pages, total)
        : doc.getPageIndices();
  ...
  async highlight(
    ...
    if (
      ![options.x, options.y, options.width, options.height].every((v) =>
        Number.isFinite(v),
      )
    ) {
    ...
    const opacity = Number.isFinite(options.opacity)
      ? Math.max(0, Math.min(1, options.opacity as number))
      : 0.35;
  ...
  async addPageNumbers(
    ...
    const startAt = Number.isFinite(options.startAt)
      ? (options.startAt as number)
      : 1;
    const fontSize = Number.isFinite(options.fontSize)
      ? (options.fontSize as number)
      : 10;
    const margin = Number.isFinite(options.margin)
      ? (options.margin as number)
      : 24;
  ...
  async watermark(
    ...
    const rotation = degrees(
      this.normalizeAngle(options.rotationDegrees ?? -35),
    );
    const fontSize = Number.isFinite(options.fontSize)
      ? (options.fontSize as number)
      : 72;
  ...
  async stamp(
    ...
    const pageIndex = Number.isFinite(options.page)
      ? Math.max(0, (options.page as number) - 1)
      : 0;
    if (pageIndex < 0 || pageIndex >= pages.length) {
  ...
  async redactByRasterize(
    ...
    for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
      ...
      const pageRedactions = options.redactions.filter(
        (r) => r.page === pageIndex + 1,
      );
      const png = await sharp(buffer, { density, page: pageIndex })
        .png()
        .toBuffer();
      ...
      if (imageWidth <= 0 || imageHeight <= 0) {
        throw new BadRequestException(
          "Failed to determine rasterized PDF page size",
        );
      }
      ...
      for (const r of pageRedactions) {
        ...
        if ([r.x, r.y, r.width, r.height].some((v) => v < 0 || v > 1)) {
          throw new BadRequestException(
            "Redaction coordinates must be normalized (0..1)",
          );
        }
