...
import { AutoRequisitionDto, CheckAvailabilityDto, InventorySyncDto } from "./dto";
import { InventoryIntegrationService } from "./inventory-integration.service";

@Controller("procurement/inventory")
@UseGuards(JwtAuthGuard, RolesGuard)
export class ProcurementInventoryController {
  constructor(private readonly inventoryIntegration: InventoryIntegrationService) {}
  ...
  sync(@Body() dto: InventorySyncDto, @CurrentUser() user: any) {
  ...
  reserve(@Param("requisitionId") requisitionId: string, @CurrentUser() user: any) {
    return this.inventoryIntegration.reserveStockForRequisition(requisitionId, user);
  }
  ...
  release(@Param("requisitionId") requisitionId: string, @CurrentUser() user: any) {
  ...
  autoRequisition(@Body() dto: AutoRequisitionDto, @CurrentUser() user: any) {
    return this.inventoryIntegration.generateReorderRequisition(dto.stockItemId, user);
  }
}
