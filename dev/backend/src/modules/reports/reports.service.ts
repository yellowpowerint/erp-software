...
export class ReportsService {
  ...
  async getFinancialSummary(startDate?: Date, endDate?: Date) {
    ...
    const totalBudget = budgets.reduce(
      (sum, bud) => sum + bud.allocatedAmount,
      0,
    );
  ...
  async getInventoryReport() {
    const [items, lowStock, outOfStock] = await Promise.all([
      ...
      this.prisma.stockItem.count({
        where: { currentQuantity: { equals: 0 } },
      }),
    ]);

    const totalValue = items.reduce(
      (sum, item) => sum + (item.unitPrice || 0) * item.currentQuantity,
      0,
    );
    ...
    return {
      ...
      itemsByCategory: Object.entries(itemsByCategory).map(
        ([category, count]) => ({
          category,
          count,
        }),
  ...
  async getAssetReport() {
    ...
    return {
      ...
      assetsByCategory: Object.entries(assetsByCategory).map(
        ([category, count]) => ({
          category,
          count,
        }),
  ...
  async getHRReport() {
    const [employees, attendances, leaveRequests, trainings] =
      await Promise.all([
    ...
    const employeesByStatus = employees.reduce(
      (acc, emp) => {
    ...
    return {
      ...
      employeesByDepartment: Object.entries(employeesByDepartment).map(
        ([department, count]) => ({
          department,
          count,
        }),
      ),
      employeesByStatus: Object.entries(employeesByStatus).map(
        ([status, count]) => ({
          status,
          count,
        }),
  ...
  async getSafetyReport(startDate?: Date, endDate?: Date) {
    ...
    const inspectionsByStatus = inspections.reduce(
      (acc, insp) => {
    ...
    const totalParticipants = trainings.reduce(
      (sum, t) => sum + (t.completedBy?.length || 0),
      0,
    );
    ...
    const averageDrillRating =
      drills.length > 0
        ? drills.reduce((sum, d) => sum + (d.successRating || 0), 0) /
          drills.length
        : 0;
    ...
    return {
      ...
      inspectionsByStatus: Object.entries(inspectionsByStatus).map(
        ([status, count]) => ({
          status,
          count,
        }),
