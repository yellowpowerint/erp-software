...
export class SettingsController {
  ...
  async resetUserPassword(@Param('id') id: string, @Body() data: { newPassword: string }) {
    return this.settingsService.resetUserPassword(id, data.newPassword);
  }

  @Post('change-password')
  async changePassword(@Body() data: { userId: string; currentPassword: string; newPassword: string }) {
    return this.settingsService.changePassword(data.userId, data.currentPassword, data.newPassword);
  }
