...
export function DashboardInsightsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await http.get('/ai/dashboard-insights');
      setData(res.data);
      setLastUpdated(new Date());
    } catch (e: any) {
      const parsed = parseApiError(e, API_BASE_URL);
      const statusPart = parsed.status ? ` (${parsed.status})` : '';
      setError(`Failed to load insights${statusPart}: ${parsed.message}`);
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      {sections.length ? (
        ...
        <View style={styles.center}>
          <Text style={styles.muted}>No insights available.</Text>
        </View>
      )}
    </ScrollView>
  );
