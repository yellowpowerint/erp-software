...
import { DollarSign, Plus, Search, Filter, Eye } from 'lucide-react';
...
function PaymentRequestsContent() {
  ...
  const fetchRequests = async () => {
    try {
      const response = await api.get('/approvals/payment-requests');
  ...
  const canCreate = user && ['SUPER_ADMIN', 'CEO', 'CFO', 'ACCOUNTANT', 'DEPARTMENT_HEAD'].includes(user.role);

  const filteredRequests = requests.filter((req) => {
    const matchesSearch = req.requestNumber.toLowerCase().includes(searchQuery.toLowerCase()) ||
      req.payeeName.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <DashboardLayout>
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage payment vouchers and reimbursements</p>
          </div>
          {canCreate && (
            <Link
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by request number or payee..."
              value={searchQuery}
      ...
      {loading ? (
        <div className="text-center py-12 bg-white rounded-lg shadow">
          ...
          <p className="text-gray-600 mt-4">Loading payment requests...</p>
        </div>
      ) : (
              ...
              {filteredRequests.length === 0 ? (
                ...
                filteredRequests.map((request) => (
                  <tr key={request.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 capitalize">{request.paymentType.replace('_', ' ')}</td>
                    <td className="px-6 py-4 text-sm text-gray-900">{request.payeeName}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">₵{request.amount.toLocaleString()}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <Link
                        href={`/approvals/payment-requests/${request.id}`}
