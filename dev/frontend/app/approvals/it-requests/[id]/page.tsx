...
import { Laptop, ArrowLeft, CheckCircle, XCircle, Clock, Calendar, DollarSign, User, MessageSquare } from 'lucide-react';
...
function ITRequestDetailContent() {
  ...
  const fetchRequest = async () => {
    try {
      const response = await api.get(`/approvals/it-requests/${params.id}`);
      setRequest(response.data);
    } catch (error) {
      console.error('Failed to fetch IT request:', error);
      alert('IT request not found');
  ...
  const handleApprove = async () => {
    ...
    try {
      await api.post(`/approvals/it-requests/${params.id}/approve`, { comments });
      alert('IT request approved successfully!');
      setShowApproveModal(false);
      fetchRequest();
    } catch (error: any) {
      console.error('Failed to approve IT request:', error);
      alert(error.response?.data?.message || 'Failed to approve IT request');
    } finally {
  ...
  const handleReject = async () => {
    ...
    setActionLoading(true);
    try {
      await api.post(`/approvals/it-requests/${params.id}/reject`, { comments });
      alert('IT request rejected');
      setShowRejectModal(false);
      fetchRequest();
    } catch (error: any) {
      console.error('Failed to reject IT request:', error);
      alert(error.response?.data?.message || 'Failed to reject IT request');
    } finally {
      setActionLoading(false);
    }
  };

  const canApprove = user && ['SUPER_ADMIN', 'CEO', 'CFO', 'IT_MANAGER'].includes(user.role);
    ...
    return (
      <DashboardLayout>
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mt-4">Loading IT request...</p>
        </div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium text-gray-900 capitalize">{request.requestType.replace('_', ' ')}</p>
                </div>
                <div>
                  <label className="text-sm text-gray-500">Priority</label>
                  <p className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${getPriorityBadge(request.priority)}`}>
                    {request.priority}
                  </p>
                </div>
              </div>
              ...
              <div>
                <label className="text-sm text-gray-500">Business Justification</label>
                <p className="text-gray-900 whitespace-pre-wrap">{request.justification}</p>
              </div>

              {request.estimatedCost && (
                <div>
                  <label className="text-sm text-gray-500">Estimated Cost</label>
                  <p className="font-medium text-gray-900">{request.currency} {request.estimatedCost.toLocaleString()}</p>
                </div>
              )}
      ...
      {showApproveModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Approve IT Request</h3>
            <p className="text-gray-600 mb-4">Are you sure you want to approve this IT request?</p>
            <div className="mb-4">
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Reject IT Request</h3>
...
export default function ITRequestDetailPage() {
  return (
    <ProtectedRoute>
      <ITRequestDetailContent />
    </ProtectedRoute>
  );
}
