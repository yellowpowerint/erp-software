...
function SafetyContent() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get('/procurement/reports/safety', {
        params: { startDate: startDate || undefined, endDate: endDate || undefined },
      });
      setData(res.data);
    } catch (e) {
      console.error('Failed to fetch safety equipment report:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Safety Equipment</h1>
        <p className="text-gray-600 mt-1">Invoice items linked to safety gear stock category</p>
      </div>
      ...
      {loading ? (
        <div className="text-center py-12 bg-white rounded-lg shadow">
          ...
          <p className="text-gray-600 mt-4">Loading safety equipment...</p>
        </div>
      ) : (
        <>
