...
**Status**: ✅ Prompt 1 Complete | ✅ Prompt 2 Complete (Verification & Integration)
...
**Prompt 2 (Verification & Integration):**
- `dev/mobile/App.tsx` - Initialize Sentry + wrap with ErrorBoundary
- `dev/mobile/src/store/authStore.ts` - Set/clear Sentry user context on auth lifecycle
- `dev/mobile/src/services/api.service.ts` - Re-export monitored apiClient
- `dev/mobile/src/services/api.service.enhanced.ts` - Allow setToken(null)
- `dev/mobile/src/config/sentry.config.ts` - Remove console.warn

## Prompt 2 Integration Complete ✅

**What was wired:**
- ✅ Sentry initialized at app startup (`App.tsx`)
- ✅ ErrorBoundary wraps entire app tree
- ✅ User context set on login/bootstrap (ID + role only, no PII)
- ✅ User context cleared on logout/session failure
- ✅ All services now use monitored `apiClient` (breadcrumbs + error capture)
- ✅ TypeScript compilation passes
- ✅ No console warnings

**Git Commit**: `af7de85` - "feat(M6.2): Wire Sentry monitoring into app (Prompt 2 verification)"

## Next Steps

1. **Get Sentry DSN**: Create project at sentry.io
2. **Configure .env**: Add EXPO_PUBLIC_SENTRY_DSN
3. **Test monitoring**: Trigger test error and verify Sentry capture
4. **Verify privacy**: Inspect Sentry events for PII/tokens
5. **Monitor dashboard**: Track crash-free rate baseline
6. **Set up alerts**: Configure Sentry notifications
