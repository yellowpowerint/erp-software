...
# Test the connection
$testResult = ssh -o BatchMode=yes ${DEPLOY_USER}@${SERVER_IP} "echo 'SSH key authentication working!'" 2>&1

if ($LASTEXITCODE -eq 0) {
    Write-Host "✓ SSH key authentication test successful!" -ForegroundColor Green
} else {
    Write-Host "⚠ SSH key test failed. Try connecting manually." -ForegroundColor Red
}
